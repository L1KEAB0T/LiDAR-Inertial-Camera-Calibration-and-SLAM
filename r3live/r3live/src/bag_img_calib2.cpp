...
int main(int argc, char **argv) {
  ...
  ros::NodeHandle nh;
  nh.param<string>("bag2pcd/bag_file", bag_file, "");
  nh.param<string>("bag2pcd/lidar_topic", lidar_topic, "/livox/lidar");
  nh.param<bool>("bag2pcd/is_custom_msg", is_custom_msg, true);
  ...
  Calibration calibra(image_file, output_cloud, calib_config_file);
...
// Convert Eigen::Matrix3d to std::vector<double>
std::vector<double> R_vec(9);
Eigen::Map<Eigen::Matrix3d>(R_vec.data()) = R;  // Convert Eigen Matrix3d to std::vector
ros::param::set("camera_extrinsics/R", R_vec);

// Convert Eigen::Vector3d to std::vector<double>
std::vector<double> T_vec(3);
Eigen::Map<Eigen::Vector3d>(T_vec.data()) = T;  // Convert Eigen Vector3d to std::vector
ros::param::set("camera_extrinsics/T", T_vec);
// 从 ROS 参数服务器获取参数
std::vector<double> new_R_vec, new_T_vec;
bool r_success = ros::param::get("/camera_extrinsics/R", new_R_vec);
bool t_success = ros::param::get("/camera_extrinsics/T", new_T_vec);

if (r_success && t_success)
{
    ROS_INFO("Parameters loaded successfully:");

    // Convert std::vector to Eigen matrix/vector using Eigen::Map
    if (new_R_vec.size() == 9 && new_T_vec.size() == 3)
    {
        Eigen::Map<Eigen::Matrix3d> R_map(new_R_vec.data());  // Use Eigen::Map to map the data
        Eigen::Map<Eigen::Vector3d> T_map(new_T_vec.data());

        R = R_map;  // Convert Eigen::Map to Eigen::Matrix3d
        T = T_map;  // Convert Eigen::Map to Eigen::Vector3d

        ROS_INFO_STREAM("Rotation Matrix: \n" << R);
        ROS_INFO_STREAM("Translation Vector: " << T.transpose());
    }
    else
    {
        ROS_ERROR("Parameter size mismatch.");
    }
}
else
{
